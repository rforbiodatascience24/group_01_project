...
ggplot(kegg_up, aes(x = Count, y = reorder(Description, -p.adjust), fill = p.adjust)) +
  ...
  theme_prism() +
  theme(
    plot.title = element_text(hjust = 0.9)  # Move title to the left
  ) +
  guides(fill = guide_colorbar(reverse = TRUE)) +  # Flip the color scale in the legend
  geom_text(aes(label = scientific(p.adjust)), hjust = -0.1, color = "black")  # Format p
